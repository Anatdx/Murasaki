...
public class BinderSender {
    ...
    private static final String PERMISSION_MANAGER = "com.anatdx.murasaki.permission.MANAGER";
    private static final String PERMISSION = "com.anatdx.murasaki.permission.API_V23";

    private static MurasakiService sMurasakiService;
    ...
    private static void sendBinder(int uid, int pid) throws RemoteException {
        ...
        for (String packageName : packages) {
            ...
            if (ArraysKt.contains(pi.requestedPermissions, PERMISSION_MANAGER)) {
                ...
                if (granted) {
                    MurasakiService.sendBinderToManger(sMurasakiService, userId);
                    return;
                }
            } else if (ArraysKt.contains(pi.requestedPermissions, PERMISSION)) {
                MurasakiService.sendBinderToUserApp(sMurasakiService, packageName, userId);
                return;
            }
        }
    }

    public static void register(MurasakiService murasakiService) {
        sMurasakiService = murasakiService;
